...
$stmt= $con->prepare("SELECT r.typeOfReport, asr.start_date, asr.end_date, r.type ,count(r.typeOfReport) as type_count FROM admin_summary_report asr INNER JOIN reports r ON r.rid = asr.rid WHERE asr.start_date <= LAST_DAY(asr.end_date) AND r.type='Hazard' GROUP BY r.typeOfReport");
